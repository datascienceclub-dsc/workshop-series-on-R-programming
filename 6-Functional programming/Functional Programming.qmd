...
So far we've been thinking of `lambda` as fixed & known & the function gives us the *probability of getting different values of `x`*. But in real-life, we observe `x` and it is `lambda` that is unknown. We want to find the `lambda` that makes the observed `x` the most likely. That is, given `x`, what value of `lambda` gives us the highest value of `log_prob_poisson`()?
